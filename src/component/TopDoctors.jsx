  ...
  return (
    <div className=' flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10 '>
        ...
        <div className=" w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0">
                ...
                <div onClick={()=>{navigate(`/appointment/${item._id}`);scrollTo(0,0)}} key={index} className=" border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500">
                    ...
                    <div className="p-4">
                        <div className="flex items-center gap-2 text-center text-green-500">
                            <p className='w-2 h-2 bg-green-600 rounded-full'></p><p>Avaliable</p>
                        </div>
                        <p>{item.name}</p>
            ...
            <button onClick={()=>{navigate('/doctors'); scrollTo(0,0)}} className='bg-blue-100 text-gray-600 px-12 py-3 rounded-full mt-10 cursor-pointer hover:scale-105 transition-all duration-300'>More</button>
    </div>
  )
}
